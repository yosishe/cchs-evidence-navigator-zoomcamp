...
NUMBERED_PROMPTS = frozenset({"numbered_evidence", "numbered_precise"})
SPAN_PROMPTS = frozenset({"source_spans_concise", "source_spans_complete"})
EXTRACT_PROMPTS = frozenset({"source_extract_concise", "source_extract_complete", "source_extract_checked", "source_extract_reasoned"})


def verify_answer_style(record):
    expected = ("verbatim_source_selection" if record.get("mode") == "llm"
                and record.get("config", {}).get("prompt") in EXTRACT_PROMPTS else None)
    if record.get("answer_style") != expected:
        raise ValueError("Answer style differs from the configured generation route")
    if expected and any(c.get("text") != c.get("exact_quote") for c in record.get("claims", [])):
        raise ValueError("A source-excerpt answer contains altered scientific prose")
    return expected
...
def validate_output_format_config(config):
    """Reject ambiguous local format experiments before any model attempt."""
    if type(config.get("json_mode", True)) is not bool:
        raise ValueError("json_mode must be a boolean")
    schema = config.get("generation_json_schema", False)
    if type(schema) is not bool:
        raise ValueError("generation_json_schema must be a boolean")
    if schema and (config.get("provider", "openai") != "ollama"
                   or not config.get("json_mode", True)):
        raise ValueError("Generation schema requires Ollama with json_mode enabled")
...
def load_config(path: Path | None = None):
    ...
    validate_output_format_config(cfg)
    ...
    if cfg["prompt"] not in {"concise", "evidence_first", "coverage_first", "course_user", "schema_first", "plain_context", "short_ids", "compact_metadata"} | NUMBERED_PROMPTS | SPAN_PROMPTS | EXTRACT_PROMPTS:
