...
def summarize_answer_review(results, reviews):
    ...
    for row in rows:
        ...
        by_prompt[row["prompt"]].append({"acceptable": bool(acceptable), "relevance": review["relevance"],
                                        ...
                                        "fact_coverage": review.get("fact_coverage"),
    ...
    for prompt, values in by_prompt.items():
        ...
        aggregates.append({"prompt": prompt, "evaluated_questions": len(values),
            ...
            "coverage_diagnostics": coverage_diagnostics(values, completeness_available)})
    slices = []
    for prompt, values in by_prompt.items():
        for slice_name in sorted({v["slice"] for v in values}):
            ...
            slices.append({"prompt": prompt, "slice": slice_name,
                           ...
                           "coverage_diagnostics": coverage_diagnostics(group, completeness_available)})
    ...
    # Correct refusals remain in the metrics, but cannot by themselves establish
    # a useful answer generator on a mixed answerable/unanswerable bank.
    answered_prompts = {d["prompt"] for d in details if d["answerable"] and d["acceptable"]}
    viable = [a for a in aggregates if a["prompt"] in answered_prompts]
    best = max((a["acceptable_rate"] or 0 for a in viable), default=0)
    return {"status": "REVIEWED_COMPLETE" if complete else "REVIEWED_INCOMPLETE_NO_SELECTION",
        ...
        "eligible_best_prompts": [a["prompt"] for a in viable if a["acceptable_rate"] == best] if complete and best > 0 and results.get("split", "tuning") == "tuning" else [],
        ...
        "selection_rule": "Highest supported, complete and relevant response rate on tuning data among candidates with at least one acceptable answerable response. Refusals alone, legacy incomplete-reference and test-only results cannot nominate a winner. This is necessary, not sufficient for research readiness. No automatic promotion.",
...
def coverage_diagnostics(rows, available=True):
    """Unweighted counts explain partial progress; never substitute for acceptance.

    Empty requirements have denominator zero and rate null, not perfect coverage.
    Unsupported/uncertain claims and whole-answer vetoes still block acceptance.
    """
    output = {"schema_version": 1, "purpose": "DIAGNOSTIC_ONLY_NOT_A_SELECTION_SCORE"}
    for field in ("fact_coverage", "qualification_coverage"):
        labels = [label for row in rows for label in (row.get(field) or [])] if available else []
        counts = {label: labels.count(label) for label in ("covered", "partial", "missing", "uncertain")}
        output[field] = {"available": available, "denominator": len(labels) if available else None,
            "counts": counts if available else None,
            "fully_covered_rate": counts["covered"] / len(labels) if labels else None}
    return output
