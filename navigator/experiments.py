...
from .rag import build_context, generate, prepare_context, validate_answer, generation_messages, generation_instructions, citation_aliases, decode_model_answer, PROMPTS, OUTPUT_RULES
...
def verify_candidate_record(row, question, config, documents, code_id):
    ...
    """
    record = row["record"]
    from .common import verify_answer_style
    ...
    call = calls[0]
    instructions = generation_instructions(expected_config)
    ...
    answer = validate_answer({k: record[k] for k in ("status", "claims", "limitations")}, hits)
    resolutions = []
    if decode_model_answer(call["raw_output"], hits, expected_config, resolutions=resolutions) != answer:
        raise ValueError("Reviewed answer differs from the validated raw output")
    if record.get("citation_resolutions", []) != resolutions:
        raise ValueError("Citation resolution receipt differs from raw output and source offsets")
    return context_id
...
def select_generation(result_paths, review_paths, *, apply=False, calibration_path=None):
    ...
    for results_path, reviews_path in zip(result_paths, review_paths):
        ...
        for aggregate in summary["aggregates"]:
            ...
            answerable_successes = sum(d["answerable"] and d["acceptable"]
                                       for d in summary["per_question"]
                                       if d["prompt"] == aggregate["prompt"])
            candidates.append({**aggregate, "model": cfg["model"],
                               "acceptable_answerable_answers": answerable_successes})
    ...
    viable = [c for c in viable if c["acceptable_answerable_answers"] > 0]
    if not viable:
        raise ValueError("No acceptable answerable response; correct refusals alone cannot establish a useful winner")
    ...
    artifact = {"status": "SELECTED_ON_PROVISIONAL_AGENT_REVIEW", "created_at": utc_now(),
                ...
                "rule": "Require no critical or unsupported-claim failures and at least one acceptable answerable response, then highest acceptable answer rate; ties use unsupported claims, errors, latency, then stable names. These are necessary selection conditions, not research-readiness certification.",
