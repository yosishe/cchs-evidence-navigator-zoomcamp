...
def attach_claims_only(raw, hits):
    """Prototype transport adapter; factual caveats must stay in claim text.

    No model prose is silently removed. Extra top-level fields are rejected.
    Empty claims describe this request's output, not absence from all literature.
    """
    value = json.loads(raw)
    if (not isinstance(value, dict) or set(value) != {"claims"}
            or not isinstance(value["claims"], list)):
        raise ValueError("The claims-only prototype requires exactly a claims list")
    answered = bool(value["claims"])
    note = ("The cited source windows are attached in full. Human review is pending."
            if answered else "No sourced answer was produced from the retrieved passages. "
            "This does not establish absence from the wider literature. Human review is pending.")
    return attach_source_windows(json.dumps({"claims": value["claims"],
        "status": "answered" if answered else "insufficient_evidence", "limitations": note}), hits)
...
def check_adapter():
    ...
    minimal = {"claims": good["claims"]}
    assert attach_claims_only(json.dumps(minimal), [hit])["claims"] == answer["claims"]
    empty = attach_claims_only('{"claims": []}', [hit])
    assert empty["status"] == "insufficient_evidence" and not empty["claims"]
    assert "does not establish absence" in empty["limitations"]
    for key in ("status", "limitations", "source_url", "exact_quote"):
        extra = {**minimal, key: "Synthetic unsupported assertion; must not be dropped."}
        try:
            attach_claims_only(json.dumps(extra), [hit])
        except ValueError:
            continue
        raise AssertionError("Extra model prose was silently removed")
    for bad in ({"claims": None}, {"claims": [{"text": "Invented", "evidence_id": "E9"}]},
                {"claims": [{**good["claims"][0], "exact_quote": "Invented"}]}):
        try:
            attach_claims_only(json.dumps(bad), [hit])
        except ValueError:
            continue
        raise AssertionError("Invalid claims-only field accepted")
    # A true attached quote is still no proof of a false claim's entailment.
    assert attach_claims_only(json.dumps({"claims": false_claim["claims"]}), [hit])["claims"]
...
def run(plan_path=None):
    ...
    adapter_mode = plan.get("adapter_mode", "source_attached")
    if adapter_mode not in {"source_attached", "claims_only"}:
        raise ValueError("Unknown prototype adapter")
    adapter = attach_claims_only if adapter_mode == "claims_only" else attach_source_windows
    cfg = {**plan["config"], "prompt": "prototype_" + adapter_mode}
    identity = model_identity(cfg)
    original = read_json(ROOT / plan["source_run"])
    if plan.get("source_results_hash") and digest(original) != plan["source_results_hash"]:
        raise ValueError("Frozen source experiment changed")
    if [q["id"] for q in original["questions"]] != plan["question_ids"]:
        raise ValueError("Question order differs from the declared prototype")
    ...
    for q in report["questions"]:
        ...
        try:
            ...
            rec["raw_output"] = raw
            rec.update(adapter(raw, rec["hits"]))
